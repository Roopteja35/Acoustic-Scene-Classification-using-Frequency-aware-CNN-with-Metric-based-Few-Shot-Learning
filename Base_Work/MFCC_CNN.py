...
def extract_features(file_path, n_mfcc=13, hop_length=512, n_fft=2048):
    y, sr = librosa.load(file_path)
    mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=n_mfcc, hop_length=hop_length, n_fft=n_fft)
    return mfcc

# Function to load data and labels
def load_data_and_labels(data_dir, feature_extractor=extract_features, feature_params={}):
    data = []
    labels = []
        ...
        if os.path.isdir(folder_path):
            for filename in os.listdir(folder_path):
                ...
                if file_path.endswith(".wav"):
                    features = feature_extractor(file_path, **feature_params)
...
data, labels, classes = load_data_and_labels(data_dir, feature_extractor=extract_features, feature_params={"n_mfcc": 13})
...
model.add(layers.Conv2D(32, (3, 3), activation='relu', input_shape=(X_train.shape[1], X_train.shape[2], 1), padding='same'))
