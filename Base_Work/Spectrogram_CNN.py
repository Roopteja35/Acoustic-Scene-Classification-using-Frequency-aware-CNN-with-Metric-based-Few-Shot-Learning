...
def extract_features(file_path, hop_length=512, n_fft=2048):
...
# Visualize spectrogram for one audio in each scene on a single plot
...
for i, scene_class in enumerate(classes):
    ...
    librosa.display.specshow(librosa.amplitude_to_db(example_features, ref=np.max), x_axis='time', y_axis='log')
    plt.colorbar(format='%+2.0f dB')
    plt.title(f'Spectrogram - {scene_class}')
